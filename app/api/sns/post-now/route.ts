...
export async function POST(req: NextRequest) {
  ...
  if (!template_id || !platforms?.length)
    return NextResponse.json({ error: '템플릿과 플랫폼을 선택해주세요' }, { status: 400 });
  ...
  for (const platform of platforms as Platform[]) {
    ...
    if (!conn) {
      ...
      await supabase.from('sns_post_logs').insert({
        user_id: user.id, template_id, platform, status: 'failed', error_message: '연결되지 않은 플랫폼',
      });
      continue;
    }
    ...
    try {
      const { id: platformPostId } = await postToPlatform(
        platform, conn.access_token, conn.platform_user_id || '', template.content,
      );
      results.push({ platform, success: true });
      await supabase.from('sns_post_logs').insert({
        user_id: user.id, template_id, platform, status: 'success', platform_post_id: platformPostId,
      });
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      results.push({ platform, success: false, error: message });
      await supabase.from('sns_post_logs').insert({
        user_id: user.id, template_id, platform, status: 'failed', error_message: message,
      });
    }
  }
