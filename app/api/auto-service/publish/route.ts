...
export async function POST(req: NextRequest) {
  ...
  let body: { article_id?: string; blog_platforms?: string[]; sns_platforms?: string[]; wp_site_ids?: string[]; backlink_platforms?: string[]; naver_cafe_menu_id?: string; naver_cafe_open_yn?: string };
  try { body = await req.json(); } catch { return NextResponse.json({ error: '요청 파싱 실패' }, { status: 400 }); }
  const { article_id, blog_platforms = [], sns_platforms = [], wp_site_ids = [], backlink_platforms = [], naver_cafe_menu_id, naver_cafe_open_yn = 'Y' } = body;
  ...
  for (const platform of blog_platforms) {
    try {
      if (platform === 'naver') {
        ...
        const data = await res.json();
        results.naver = res.ok ? { success: true, url: data.postUrl } : { success: false, error: data.error };
