...
/**
 * AI 글쓰기 프롬프트에 워터마크 방지 지시문 추가
 */
export const ANTI_WATERMARK_PROMPT = `
[워터마크 절대 금지 - 매우 중요]
- 절대로 제로폭 공백(U+200B, U+200C, U+200D 등) 삽입 금지
- HTML 엔티티(&amp;, &lt;, &nbsp; 등) 사용 금지 — 일반 텍스트로 작성
- 숨겨진 유니코드 문자 삽입 금지
- AI 탐지 회피용 특수 패턴 사용 금지
- 동일한 문장 구조 반복 최소화 ("~입니다", "~합니다" 연속 사용 금지)
- "또한", "더불어", "아울러", "결론적으로" 같은 AI 특유 전환어 남발 금지
- 자연스러운 구어체 문장 사용 — 실제 사람이 쓴 것처럼
`.trim();
