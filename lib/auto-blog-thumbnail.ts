...
function generateSvg(title: string, keyword: string, colorScheme: 'blue' | 'dark' | 'green' = 'blue'): string {
...
<svg width="1080" height="1080" xmlns="http://www.w3.org/2000/svg">
  ...
  <!-- 키워드 태그 -->
  <rect x="${540 - (keyword.length * 14 + 40) / 2}" y="${startY + lines.length * lineHeight + 30}"
    width="${keyword.length * 14 + 40}" height="56"
    rx="28" fill="rgba(240,180,41,0.9)"/>
  <text x="540" y="${startY + lines.length * lineHeight + 64}"
    text-anchor="middle"
...
export async function generateAndUploadThumbnail(
  ...
  try {
    const svg = generateSvg(title, keyword, colorScheme);
