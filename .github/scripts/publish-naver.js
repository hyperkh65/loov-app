...
function htmlToParagraphs(html) {
  ...
  // 이어지는 걸 확인함. 구두점 뒤 스페이스를 NBSP로 바꿔봐도(직전 시도) 여전히
  // 변환이 일어나는 걸 실사용으로 재확인함 — 감지 정규식이 "숫자+구두점" 인접
  // 자체를 보고 트리거되는 것으로 보임. 그래서 이번엔 숫자와 구두점 "사이"에
  // 폭 0 문자를 끼워넣어 인접 패턴 자체를 깬다(화면엔 안 보이지만 정규식과는
  // 더 이상 붙어서 매치되지 않음).
